...
class GetUserDetailsAction(Action):
    def run(self):
        # Azure AD details (Replace with your actual values)
        ...
        def get_all_users_enabled_within_expiry_range():
            users = []
        ...
        # Function to generate real email addresses based on the 'Email' field in the CSV
        def generate_real_email_addresses(users):
            return [f"{user['name'].replace(' ', '').lower()}@{user['email'].split('@')[-1]}" for user in users]
        ...
        users = get_all_users_enabled_within_expiry_range()

        # Process user details
        result = {
            'user_details': users,
            'email_addresses': generate_real_email_addresses(users)
        }
