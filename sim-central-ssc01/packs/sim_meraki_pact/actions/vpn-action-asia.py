...
def sync_sitesid():
    ...
    db_cursor.execute('DELETE FROM pact_asia_vlans;')
    db_connection.commit()
    db_connection.close()
    try:
        pact_asia_networks = dashboard.organizations.getOrganizationNetworks(org_id)
        for net in pact_asia_networks:
            site_subnet=[]
            network_id = net['id']
            network_name= net['name']
            print(f'{network_name}\n')
            try:
                pact_asia_subnets = dashboard.appliance.getNetworkApplianceVlans(network_id)
                for subnet in pact_asia_subnets:
            ...
            db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db')
            db_cursor = db_connection.cursor()
            db_cursor.execute('INSERT INTO pact_asia_vlans(network_id, network_name,network_vlans) values (?,?,?)',(str(network_id),str(network_name),str(site_subnet_details)))
            db_connection.commit()
            db_connection.close()
    except meraki.exceptions.APIError as e:
        print(e)
...
def restore_multisite():
    ...
    db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db')
    db_cursor = db_connection.cursor()
    isolated_site_names = db_cursor.execute('SELECT network_name from pact_asia_isolatedsites WHERE network_name=?', (sitename,)).fetchall()
    if len(isolated_site_names) != 0:
        print(f'Site {isolated_site_names} isolated. Running process to restore site. \n')
        db_cursor.execute('DELETE from pact_asia_isolatedsites WHERE network_name=?', (sitename,))
        db_connection.commit()
        db_connection.close()
        ...
        db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db')
        db_cursor = db_connection.cursor()
        isolated_network_lists = db_cursor.execute('SELECT network_id from pact_asia_isolatedsites').fetchall()
        db_connection.close()
...
def restore_singlesite():
    ...
    db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db')
    db_cursor = db_connection.cursor()
    db_cursor.execute('DELETE FROM pact_asia_isolatedsites;')
    db_connection.commit()
    db_connection.close()
...
def isolate_firstsite():
    ...
    db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db')
    db_cursor = db_connection.cursor()
    site_network_id = db_cursor.execute('SELECT network_id from pact_asia_vlans WHERE network_name=?', (sitename,)).fetchall()
    ...
    try:
        ...
        print(e)
        pass
    db_cursor.execute('INSERT INTO pact_asia_isolatedsites(network_id, network_name) values (?,?)',(str(site_network_id_final),str(sitename)))
    db_connection.commit()
    db_connection.close()
...
if __name__ == '__main__':
    ...
    db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db') 
    db_cursor = db_connection.cursor()
    db_cursor.execute('''CREATE TABLE IF NOT EXISTS pact_asia_vlans ([network_id] VARCHAR, [network_name] VARCHAR,[network_vlans] VARCHAR(255))''')
    db_cursor.execute('''CREATE TABLE IF NOT EXISTS pact_asia_isolatedsites ([network_id] VARCHAR, [network_name] VARCHAR)''')   
    db_connection.commit()
    db_connection.close()
    ...
    #Sync PACT Asia Sites and Subnets to DB
    org_id = '669347494617942420'
    org_name="PACT Group Asia"
    ...
    db_connection = sqlite3.connect('/home/ssadmin/sim_meraki_pact/actions/pact_sites_iso.db') 
    db_cursor = db_connection.cursor()
    isolated_count=db_cursor.execute('SELECT * from pact_asia_isolatedsites').fetchall()
    db_connection.close()
