...
class AddUserToGroupAction(Action):
    def run(self, user_upn, group_display_name, client_id, tenant_id, client_secret):
        ...
        response = requests.get(get_user_url, headers=headers,verify=False)
        ...
        if response.status_code == 200:
            ...
            response = requests.get(get_groups_url, headers=headers,verify=False)
            ...
            if response.status_code == 200:
                ...
                for group in groups:
                    if group['displayName'] == group_display_name:
                        ...
                        response = requests.post(add_user_to_group_url, headers=headers, json=data,verify=False)
