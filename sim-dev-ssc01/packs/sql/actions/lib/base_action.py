...
class BaseAction(Action):
    ...
    def generate_bulk_where_clause(self, sql_table, sql_obj, where_dict_array):
        return_array = []
        index = 0
        for record in where_dict_array:
            return_dict = {}
        ...
        return (sql_obj, return_array)


    def generate_bulk_values(self, sql_obj, data_dict_array):
        data_array = []
    ...
    def merge_bulk_dicts(self, dicts_array):
        """ Merge Array of dictionaries into 1 single Dictionary
        and return.
        """
        result_array = []
        for dicts in dicts_array:
            result = {}
            for d in dicts:
                if d:
                    result.update(d)
