...
RESOURCE_POOL_NON_JSON_SERILIZABLE_TYPES = [
    vim.ResourcePool,
...
VM_GET_NON_JSON_SERILIZABLE_TYPES = [
    vim.vm.device,
    vim.vm.Summary,
    vim.vm.Summary.ConfigSummary,
    vim.vm.DeviceRuntimeInfo
...
class VMGetJSONEncoder(json.JSONEncoder):
    def default(self, obj):  # pylint: disable=E0202
        try:
            return super(VMGetJSONEncoder, self).default(obj)
        except TypeError:
            if type(obj) in VM_GET_NON_JSON_SERILIZABLE_TYPES:
                return obj.__dict__
...
class ResourcePoolGetJSONEncoder(json.JSONEncoder):
    def default(self, obj):  # pylint: disable=E0202
        try:
            return super(ResourcePoolGetJSONEncoder, self).default(obj)
        except TypeError:
            if type(obj) in RESOURCE_POOL_NON_JSON_SERILIZABLE_TYPES:
                return obj.__dict__
