...
class vCenterManagementGroupSensor(PollingSensor):
    def __init__(self, config, sensor_service, poll_interval=300):
        super(vCenterManagementGroupSensor, self).__init__(sensor_service=sensor_service,
                                                 config=config)
        self._logger = self.sensor_service.get_logger(name=self.__class__.__name__)
        self.trigger_name = 'vcenter_management_group_action'
    ...
    def setup(self):
        ...
        current_minute = self.closest(int(current_ast_ts.strftime('%M')))
