...
class VcenterRawData(BaseAction):
    def run(self, vsphere, vcenter_id, get_vms=False, ids=[], get_vm_disks=False, get_vm_networks=False, get_hosts=False, get_rps=False, get_folders=False, get_tags=False, datastores=[], custids=[], vm_required_field_array=[], vmdisk_required_field_array=[], tag_filter=[]):
    ...
    def _add_vm_properties_to_map_from_vm_array(self, vm_map, vm_array):
        for vm in vm_array:
            ...
            vm_map[vm._GetMoId()] = {
                "_ethvmid": ethvmid,
                "_vmID": vm_id,
                "_custAnno": "".join([x.value for x in vm.customValue if x.key == customer_id]),
        ...
        for network in networks:
            if network.deviceConfigId > 0:
                ...
                if hasattr(network.ipConfig, 'ipAddress'):
                  network_obj['ipAddress']   = "".join([ip.ipAddress for ip in network.ipConfig.ipAddress if ip.state == 'preferred'])
                else:
